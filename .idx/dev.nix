{ pkgs, ... }: {
  ...
  idx.workspace.onCreate = {
    bootstrap-ssh = ''
      ...
      echo "[bootstrap-ssh] Creating SSH tunnel startup script..."
      
      # Create the tunnel script
      cat > /tmp/start-ssh-tunnel.sh <<'TUNNEL_SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
...
TUNNEL_SCRIPT
      ...
      chmod +x /tmp/start-ssh-tunnel.sh
      echo "[bootstrap-ssh] Executing tunnel script..."
      /tmp/start-ssh-tunnel.sh
    '';
  };
}
